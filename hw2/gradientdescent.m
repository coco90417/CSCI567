...
for k = 1:size(T,2)
    ...
    train_data_cross_entropy(i, j, k) = -(transpose(train_label) * log(sigma_train) + transpose(ones(size(train_label,1),1)-train_label) * log(ones(size(train_label,1),1)-sigma_train)) + lambda(i)*norm(w)^2;
