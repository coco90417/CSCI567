...
for k = 1:size(T);
    gradient = transpose(new_train_data) * (sigmoid(new_train_data * transpose(new_w))-train_label) + 2*lambda(i)*lambda_w;
    new_w = new_w - stepsize(j) * gradient;
    lambda_w = [0; new_w(2:size(new_w,2))];
    train_data_cross_entropy[i, j, k] = transpose(train_label) * (sigmoid(new_train_data * transpose(new_w))-test_label) + 2*lambda(i)*norm(lambda_w);
    test_data_cross_entropy[i, j, k] = transpose(test_label) * (sigmoid(new_test_data * transpose(new_w))-test_label) + 2*lambda(i)*norm(lambda_w);
