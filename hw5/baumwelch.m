...
for i=1:2
beta_t = 6-t;
beta(beta_t,i,n) = sum(A_guess(i,:)'.* B_guess(:,data(n,beta_t+1)).* beta(beta_t+1,:,n)');
...
B_estimate = zeros(2,4);
B_estimate_temp_down_one = zeros(size(data, 1),1);
...
c_four = c(data(m,:)==4,m);
B_estimate_temp_up_one(m) = sum(temp_alpha_one.*temp_beta_one ./c_one);
B_estimate_temp_up_two(m) = sum(temp_alpha_two.*temp_beta_two ./c_two);
B_estimate_temp_up_three(m) = sum(temp_alpha_three.*temp_beta_three ./c_three);
B_estimate_temp_up_four(m) = sum(temp_alpha_four.*temp_beta_four ./c_four);
B_estimate_temp_down_one(m) = sum(alpha(:,i,m).*beta(:,i,m)./c(:,m));
B_estimate_temp_down_two(m) = sum(alpha(:,i,m).*beta(:,i,m)./c(:,m));
B_estimate_temp_down_three(m) = sum(alpha(:,i,m).*beta(:,i,m)./c(:,m));
B_estimate_temp_down_four(m) = sum(alpha(:,i,m).*beta(:,i,m)./c(:,m));
end
